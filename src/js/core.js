...
(function () {
   ...
   * @param {Object} cloned - The new object copy to re-add missing references to
   * @param {Object} object - The original object
   * @param {Array} parents - An array of previously processed objects to avoid circular references
   * @param {recursiveMapCallback} fn - A reference to the function to be recursively called
   * @returns {Object|Array}
   */
   ...
   * @param {*} val - The current value being process
   * @param {Array} [parents=[]] - An array of all the previously processed elements
   * @returns {undefined|*}
   */
   ...
   * @returns {number}
   */
  const randomNumber = (range = 10.0, offset = 0.0, interval = 1.0) => (Math.random() * range + offset) * interval
   ...
   * @returns {number}
   */
  const randomInteger = (range = 10, offset = 0, interval = 1) => (Math.floor(Math.random() * range) + offset) * interval
